...
var UsersController = class {
  async create(request2, response) {
    ...
    return response.status(201).json(userWithoutPassord);
  }
};
...
function verifyUserAuthorization(role) {
  return (request2, response, next) => {
    ...
    if (!role.includes(request2.user.role)) {
      throw new AppError("Unauthorized", 401);
    }
    return next();
  };
...
var DeliveriesLogsController = class {
  async create(request2, response) {
  ...
  async show(request2, response) {
    ...
    if (request2.user?.role === "customer" && request2.user.id !== delivery?.userId) {
      throw new AppError("The user can only see their own deliveries", 401);
    }
...
// src/routes/deliveries-logs-routes.ts
var deliveriesLogsRoutes = (0, import_express4.Router)();
var deliveriesLogsController = new DeliveriesLogsController();
...
function errorHandling(error, request2, response, next) {
...
describe("SessionController", () => {
  ...
  it("Should authenticate and get an acess token", async () => {
